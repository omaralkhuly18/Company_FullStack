...
@section('content')
    <div class=" tajawal_black container-fluid">
        ...
        <div class=" tajawal_black d-sm-flex align-items-center justify-content-between mb-4">
            ...
            <a href="{{ route('dash.category.create') }}" class=" tajawal_black d-none d-sm-inline-block btn btn-primary shadow-sm"><i
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($PostCategories as $PostCategory)
                                            <tr class=" tajawal_black odd">
                                                ...
                                                @if ($app->getLocale() == 'ar')
                                                    <td>{{ $PostCategory->name_ar }}</td>
                                                    <td>{{ $PostCategory->slug_ar }}</td>
                                                @else
                                                ...
                                                <td>
                                                    <div class=" tajawal_black btn-group" role="group">
                                                        ...
                                                        <a href="{{ route('dash.category.show.update', ['id' => $PostCategory->id]) }}"
                                                            class=" tajawal_black btn btn-primary btn-sm"><i class="fas fa-edit"></i></a>
                                                        <span style="color: white">s</span>
                                                        <form method="POST"
                                                            action="{{ route('dash.category.delete', ['id' => $PostCategory->id]) }}">
                                                            @csrf
                                                            @method('DELETE')
