...
@section('content')
    <div class=" tajawal_black container-fluid">
        ...
        <div class=" tajawal_black d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class=" tajawal_black h3 mb-0 text-gray-800">{{ trans('web_trans.posts') }}</h1>
            <a href="{{ route('dash.post.create') }}" class=" tajawal_black d-none d-sm-inline-block btn btn-primary shadow-sm"><i
                    class=" tajawal_black fas fa-plus"></i> {{ trans('web_trans.create_post') }}
            </a>
        </div>
                                            ...
                                            <tr class=" tajawal_black odd">
                                                ...
                                                <td>
                                                    <div class=" tajawal_black photo-container">
                                                        <a href="" class=" tajawal_black magnific"><img
                                                                ...
                                                                alt="{{ asset('assets/uploads/posts/' . $Post->photo) }}"></a>
                                                    </div>
                                                </td>
                                                @if ($app->getLocale() == 'ar')
                                                    <td>{{ $Post->title_ar }}</td>
                                                    <td>{{ $Post->postCategory->name_ar }}</td>
                                                @else
                                                    <td>{{ $Post->title }}</td>
                                                ...
                                                <td>
                                                    <div class=" tajawal_black btn-group" role="group">
                                                        ...
                                                        <a href="{{ route('dash.post.show.update', ['id' => $Post->id]) }}"
                                                            class=" tajawal_black btn btn-primary btn-sm"><i class="fas fa-edit"></i></a>
                                                        <span style="color: white">s</span>
                                                        <form method="POST"
                                                            action="{{ route('dash.post.delete', ['id' => $Post->id]) }}">
                                                            @csrf
                                                            @method('DELETE')
