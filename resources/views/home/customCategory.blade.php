...
@section('meta_description')
    @if ($app->getLocale() == 'ar')
        <meta name="description" content="{{ $PostCategory->seo_descr_ar }}">
    @else
        <meta name="description" content="{{ $PostCategory->seo_descr }}">
    @endif
@endsection
...
@section('key')
    @if ($app->getLocale() == 'ar')
        @php
            ...
            $cleaned_description = preg_replace('/[^\p{L}\p{N}\s]+/u', ' ', $PostCategory->seo_descr_ar);
            $words = array_unique(preg_split('/[\s,;&]+/', $cleaned_description, -1, PREG_SPLIT_NO_EMPTY));
        @endphp
    @else
        @php

            $cleaned_description = preg_replace('/[^\p{L}\p{N}\s]+/u', ' ', $PostCategory->seo_descr);
            $words = array_unique(preg_split('/[\s,;&]+/', $cleaned_description, -1, PREG_SPLIT_NO_EMPTY));
        @endphp
    @endif
...
@section('og_twitter')
    @if ($app->getLocale() == 'ar')
        <meta property="og:title" content="{{ $PostCategory->seo_title_ar }}">
        <meta property="og:description" content="{{ $PostCategory->seo_descr_ar }}">
        <meta property="og:site_name" content="{{ $PostCategory->seo_title_ar }}">
        <meta name="twitter:title" content="{{ $PostCategory->seo_title_ar }}">
        <meta name="twitter:description" content="{{ $PostCategory->seo_descr_ar }}">
    @else
        <meta property="og:title" content="{{ $PostCategory->seo_title }}">
        <meta property="og:description" content="{{ $PostCategory->seo_descr }}">
        <meta property="og:site_name" content="{{ $PostCategory->seo_title }}">
        <meta name="twitter:title" content="{{ $PostCategory->seo_title }}">
        <meta name="twitter:description" content="{{ $PostCategory->seo_descr }}">
    @endif
...
@section('title')
    @if ($app->getLocale() == 'ar')
        {{ $PostCategory->seo_title_ar }}
    @else
        {{ $PostCategory->seo_title }}
    @endif
@endsection
